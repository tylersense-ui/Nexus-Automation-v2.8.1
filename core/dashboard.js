...
/** * Nexus-Apex v44.0
 * Module: Dashboard UI
 * Description: Interface de contrôle et de monitoring.
 * Logic: Affiche les statistiques en temps réel. Limite les appels ns.ps() via un cache pour éviter le lag UI.
 * Usage: Lancé par l'Orchestrateur automatiquement. (run /core/dashboard.js)
 */
export async function main(ns) {
    ns.disableLog("ALL");
    ns.ui.openTail();
    ns.ui.moveTail(10, 10);
    ns.ui.resizeTail(502, 540);
    ...
    // Variables de cache pour optimisation RAM (Ghost in the Shell)
    let tick = 0;
    ...
    while (true) {
        ...
        const pservs = allServers.filter(s => s.startsWith("nexus-node"));
        
        // Optimisation : On ne fait le lourd calcul des processus que 1 fois sur 5 (toutes les 5s)
        if (tick % 5 === 0) {
            ...
            for (const host of allServers) {
                ...
                cachedMaxRam += ns.getServerMaxRam(host);
        ...
        const ramPerc = (cachedUsedRam / cachedMaxRam) * 100 || 0;
        
        const shareBoost = 1 + (Math.pow(cachedTS, 0.7) * 0.05);

        ns.print(`┌    Nexus-Apex v44.0   ─  ${new Date().toLocaleTimeString()}  ─  LVL ${player.skills.hacking}    ┐`);
        ...
        if (caps.tix) {
            ...
            try { ns.stock.getSymbols().forEach(s => { const [sh] = ns.stock.getPosition(s); if(sh>0){ totalStock += sh*ns.stock.getBidPrice(s); count++; }}); } catch(e){}
            ns.print(`  💹 BOURSE  : ${ns.nFormat(totalStock, "$0.000a")} (${count} Trks)`);
        }
        
        ns.print(`  ✨ XP RATE : ${ns.nFormat(xpRate || 0, "0.000a")}k/s`);
        ns.print(`├──────────────────────────────────────────────────┤`);
        ns.print(`  🌐 NODES     : ${pservs.length} / 25 Online`);
        ns.print(`  💾 NETWORK   : ${ns.formatRam(cachedUsedRam)} / ${ns.formatRam(cachedMaxRam)} (${ramPerc.toFixed(1)}%)`);
        ns.print(`  [${"█".repeat(Math.floor(ramPerc / 2.5)).padEnd(40, "░")}]`);
        ...
        for (const t of targets) {
            ...
            const mPerc = ((s.moneyAvailable / s.moneyMax) * 100 || 0).toFixed(0);
            const sDiff = (s.hackDifficulty - s.minDifficulty).toFixed(1);
            const icon = (mPerc > 98 && sDiff < 0.5) ? "💸" : "🛡️";
